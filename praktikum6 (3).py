...
Original file is located at
    https://colab.research.google.com/drive/1XmYJKwl_7f0JOY7wjqLiXLh_Us_2ngJW
"""
...
#mendapatkan resolusi image
img_height = img.shape[0]
img_width = img.shape[1]
img_channel = img.shape[2]

#membuat variabel inversi
...
#membuat variabel log
img_log = np.zeros(img.shape, dtype=np.uint8)

#membuat fungsi untuk log
def log(c):
    for y in range(0, img_height):
        for x in range(0, img_width):
            red = img[y][x][0]
...
#membuat variabel inlog
img_inlog = np.zeros(img.shape, dtype=np.uint8)

#membuat fungsi inversi log
def inlog(c):
    for y in range(0, img_height):
        for x in range(0, img_width):
            red = img[y][x][0]
...
#menampilkan nth power
nthpower(50, 100)
plt.imshow(img_nthpower)
plt.title("Nth Power")
plt.show()

#membuat variabel nth root power
img_nthrootpower = np.zeros(img.shape, dtype=np.uint8)

#membuat fungsi nth root power
